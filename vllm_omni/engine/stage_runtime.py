    ...
    def _initialize_local_llm_replica(
        ...
        try:
            ...
            with self._replica_launch_lock:
                # vLLM reads some connector settings (for example
                # VLLM_MOONCAKE_BOOTSTRAP_PORT) directly from the process
                # environment while the engine-core subprocess is spawned.
                # Scope the stage-specific environment under the same launch
                # lock so concurrent stage initialization cannot leak values
                # between workers.
